...
public interface MessageRepository extends ReactiveCrudRepository<Message, Integer> {
    ...
    @NotNull
    @Override
    @Query("UPDATE messages SET is_deleted = 1, deleted_at = CURRENT_TIMESTAMP WHERE message_id = :messageId AND is_deleted = 0")
    Mono<Void> deleteById(@NotNull Integer messageId);

    @NotNull
    @Transactional
    @Query("UPDATE messages SET is_deleted = 1, deleted_at = CURRENT_TIMESTAMP WHERE conversation_id = :conversationId AND is_deleted = 0")
    Mono<Void> deleteByConversationId(@NotNull Integer conversationId);
}
